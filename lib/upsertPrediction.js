...
//   );
// }
async function uploadImageFromData(data, filename, anon_id, isBase64 = false) {
  let blob;
  ...
  const { data: uploadData, error: uploadError } = await db.storage
    .from("images")
    .upload(`public/${anon_id}/${filename}.png`, blob, {
      cacheControl: "3600",
      upsert: false,
    });
  ...
  console.log(
    `File uploaded to: ${anon_id}/${filename}. Data: ${JSON.stringify(
      uploadData
    )}`
  );
}
